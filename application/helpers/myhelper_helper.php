<?php
	...
	function get_status_user($field, $statusId){
		$ci = & get_instance();
		$ci->db->where('statusId', $statusId);
		$tahun = $ci->db->get('dtbstatususer')->row_array();
		return $tahun[$field];
	}

	function get_vendor_name($field, $vendorId){
		$ci = & get_instance();
		$ci->db->where('vendorId', $vendorId);
		$tahun = $ci->db->get('dtbvendor')->row_array();
		return $tahun[$field];
	}
